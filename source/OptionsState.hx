		...
		if(usesCheckbox) {
			if(controls.ACCEPT && nextAccept <= 0) {
				switch(options[curSelected]) {
					...
					case 'Easy Mode':
						ClientPrefs.easyMode = !ClientPrefs.easyMode;
						ClientPrefs.hellMode = false;
						ClientPrefs.fuckMode = false;
					case 'Old Remix':
						ClientPrefs.oldSong = !ClientPrefs.oldSong;
				}
				FlxG.sound.play(Paths.sound('scrollMenu'));
				reloadValues();
			}
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			...
			case 'Fuck You':
				daText = "Fuck you.";
			case 'Easy Mode':
				daText = "Turns all the mechanics off.\nUse this if you want, I'm not your mom.";
			case 'Old Remix':
				daText = "Changes the song to the old remix\nwith the old vocals.";
			}
		...
		for (i in 0...checkboxArray.length) {
			...
			if(checkbox != null) {
				...
				switch(options[checkboxNumber[i]]) {
					...
					case 'Easy Mode':
						daValue = ClientPrefs.easyMode;
					case 'Old Remix':
						daValue = ClientPrefs.oldSong;
				}
